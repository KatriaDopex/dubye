      ...
      {Array.from({ length: max }, (_, i) => (
        <div
          ...
          className={`w-1.5 h-1.5 rounded-full ${
            i < rating ? "bg-white/50" : "bg-white/10"
          }`}
        />
      ))}
    </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {DESTINATIONS.map((dest, i) => (
            <motion.div
              ...
              className="glass rounded-2xl p-5 hover:bg-white/[0.06] transition-colors duration-300"
            >
              <div className="flex items-center gap-2.5 mb-4">
                <span className="text-xl">{dest.flag}</span>
                <div>
                  <h3 className="text-[15px] font-medium">{dest.city}</h3>
                  <p className="text-white/30 text-[11px] font-light">{dest.country}</p>
                </div>
              </div>

              <div className="space-y-2.5 mb-4">
                <div className="flex items-center justify-between">
                  <span className="text-white/35 text-[10px] uppercase tracking-[0.1em] font-light">
                    Cost of living
                  </span>
                  <span className="text-white/70 text-[13px] font-light tabular-nums">
                    {dest.costIndex}
                  </span>
                </div>
                <div className="flex items-center justify-between">
              ...
              <p className="text-white/25 text-[11px] font-light leading-relaxed italic">
                {dest.tagline}
              </p>
            </motion.div>
          ))}
